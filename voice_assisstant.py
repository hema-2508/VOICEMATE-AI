...
def extract_text_from_image(image_path):
    try:
        if not os.path.isfile(image_path):
            speak("Invalid file path. Please try again.")
            return
        ...
        temporary_file = "{}.jpg".format(os.getpid())
        cv2.imwrite(temporary_file, grayscale_image)

        text = pytesseract.image_to_string(Image.open(temporary_file))
        os.remove(temporary_file)

        print("Extracted Text: \n", text)
        speak("Text extraction complete. Please check the console for the extracted text.")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if 'open stackoverflow' in query:
            ...
            speak("Please enter the path of the image you want to convert.")
            image_path = input("Please enter the path of the image you want to convert: ")
